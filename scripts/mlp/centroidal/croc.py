...
def writeTrajInPhase(phase, c, current_t, start, end, append=False):
    ...
    assert abs(nsteps - length /
               cfg.SOLVER_DT) < 1e-10, "bezier curve length should be a multiple of the solver time step, t = " + str(
                   c.max()) + " dt = " + str(cfg.SOLVER_DT)
    if append:
        assert phase.time_trajectory[
            -1] == current_t, " called method with append = True and unconsistent phase time_trajectory"
    else:
        assert len(phase.time_trajectory) == 0 and len(
            phase.state_trajectory) == 0, "called method with append = False and phase trajectory is not empty"
    t = start
    if append:
        t += cfg.SOLVER_DT
