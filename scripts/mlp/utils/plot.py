...
def addVerticalLineContactSwitch(timeline, intervals, plt, linestyle="-.", color='k'):
    ...
    for interval in intervals:
        if len(interval) > 0 and (interval[-1] <
                                  len(timeline)):  # may happen when the motion generation did not succeed completely
...
def plotALLFromWB(cs, res, display=True, save=False, path=None):
    ...
    if res.c_t.any():
        plotCOMTraj(res.t_t, res.phases_intervals, res.c_reference, res.dc_reference, res.ddc_reference, res.c_t,
                    res.dc_t, res.ddc_t)
    ...
    if list(res.effector_trajectories.values())[0].any():
        plotEffectorTraj(res.t_t, res.phases_intervals,
                         [res.effector_references, res.d_effector_references, res.d_effector_references],
                         [res.effector_trajectories, res.d_effector_trajectories, res.dd_effector_trajectories])
